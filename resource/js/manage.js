$(document).ready(function () {
  let d = new Date();
  $('#members-dbtable-name').val('convert_' + d.getFullYear());
  $('#manage-export-month').val(d.getFullYear() + '.' + String('00' + (d.getMonth() + 1)).slice(-2));
  $('#manage-export-command').val("`studentid`='TW00103008' OR `studentid`='TW00104039' OR `area`='高區' OR `course`='1'");
      ...
      try {
        ...
        for (let w = 0, classname = '', sub = [], len = da.length; w < len; w++ , classname = '') {
          ...
          sub = {
            ...
            course: da[w][headers.course] ? da[w][headers.course] : '0',
