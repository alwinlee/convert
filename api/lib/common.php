<?php
  function check_convert_db($tbname) {
  ...
  function getPDFinfo_old($d, $m, $course) {
    ...
    if ($course == '' || (strpos($course, '下午') !== false)) {
      $info.="時間：".$d."，自行前往者<span>請於".$d."下午 13:30 前報到完畢</span>。";
    } else {
      $info.="時間：".$d."，自行前往者<span>請於".$d."上午 8:00 前報到完畢</span>。";
    }
    ...
    $info.="<td style=\"width:650px;text-align:left;\">";
    $info.="<span>為敬重受皈依故，請大家準時參加。</span>";
    $info.="</td></tr>";
    ...
    $info.="<td style=\"width:650px;text-align:left;\">";
    $info.="報到方式：<span>一張報到單領取一張座位卡，憑座位卡入場。</span>";
    $info.="</td></tr>";
    ...
    $info.="</td><td style=\"width:45px;text-align:right;\"></td></tr>";
  ...
  function getPDFinfo($d, $m, $tm) {
    ...
    $info.="<tr><td style=\"width:45px;text-align:right;\">一、</td>";
    $info.="<td style=\"width:650px;text-align:left;\">";
    $info.="日期：".$d."  <span>".$tm." 前報到完畢。 為敬重受皈依故，請準時參加！</span>";
    $info.="</td></tr>";
    ...
    $info.="<tr><td style=\"width:45px;text-align:right;\">三、</td>";
    $info.="<td style=\"width:650px;text-align:left;\">";
    $info.="報到方式：<span>一張報到單領取一張受皈依貼紙，憑受皈依貼紙入場。</span>";
    $info.="</td></tr>";
    ...
    $info.="1.法會現場須全程配戴口罩，保持社交距離，並落實及配合相關防疫措施。";
    $info.="</td></tr>";
